...
techniques.
A sequential and a non run-time scheduled parallel module is implemented for comparison sake and as a test for the Mandelbrot module.
...
This interface is provided primarily by the compute\_line function which takes a line number and the id of the thread
which is making the call. The thread id is required for visualisation purposes. A constant HEIGHT is declared in the 
accompanying header file which provides a work limit for the scheduler.

% ---------------------------------------------------------------------------------------------------------------------------------------------------------------
\subsubsection*{The Algorithm}

Each pixel of the raster plane maps to a point on the complex plane.

The algorithm uses several parameters to manipulate the raster plane.
\begin{itemize}
\item \textbf{The HEIGHT and WIDTH constants:} \\
            Defines the dimensions of the raster plane. 
\item \textbf{The MAX\_ITERATIONS constant:} \\
\item \textbf{The c\_max and x\_min complex numbers:} \\
\item \textbf{The c\_factor:} \\
\end{itemize} 

The key functions used in the mandelbrot module.

\begin{itemize}
\item \textbf{compute\_line:} \\
            Iterates a single line of the raster plane specified by a perameter \(y\) and assigns the result to the plane matrix.
            
            
\item \textbf{is\_member:} \\
\item \textbf{julia\_func:} \\
\item \textbf{convert\_y\_coord:} \\
\item \textbf{convert\_x\_coord:} \\
\end{itemize}
...
\item \textbf{ws\_become\_thief: } \\
                ...
                The exclude set is initialised adding the current thread. It is passed to ws\_random\_deque, which
