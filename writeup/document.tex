...
Multi-processor computers are now commonplace for both consumers and scientists alike. 
...
Designing effective programs to utilise such hardware, using well established traditionally sequential programming tools,
...
underlying multi-processor architectures. Previous work involving the Single Assignment C~\cite{sachome} and 
S-Net~\cite{snethome} programming languages has served to inspire, and further the desire for knowledge in the subject area.

This project is motivated by the aspiration to better understand how modern computer architectures can be utilised,
and communicate these findings in a concise, comprehensive report.

In addition the author wishes to gain valuable experience in the project development and management process.
...
With regards to the Mandelbrot Set I am interested in julia-sets in which the values \(c\) and \(z\) used are expressed as a 
...
This is one by introducing a fork point which starts the threads, and a join point which 
returns program control to the main thread.
...
that such a scheme needs to be carefully considered in implementation. 

Unfortunately, due to time constraints, there is no fully complete implementation for this scheme in place.
It is worth noting that this is area alone is a significant body of work and represents a good portion
...
Here is a brief description of the test environment used when gathering the results for this section.
